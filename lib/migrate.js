 ...
 * Handles comments and avoids splitting inside strings.
 */
function splitStatements(sql) {
  // Remove single-line comments
  const cleaned = sql.replace(/--.*$/gm, '');
  // Split on semicolons, trim, and filter empties
  return cleaned
    .split(';')
    .map(s => s.trim())
    .filter(s => s.length > 0);
 ...
 * Each SQL statement is executed individually to avoid pg-meta multi-statement issues.
 */
async function runMigrations() {
  ...
  try {
    ...
    for (const file of pending) {
      ...
      try {
        // Split into individual statements to avoid pg-meta multi-statement issues
        const statements = splitStatements(sql);
        logger.info(`Migration ${file}: executing ${statements.length} statement(s)`);
