 ...
 *
 * Gracefully degrades to stateless mode if chat tables don't exist yet.
 */
...
/**
 * Try to create a new chat session. Returns null if tables don't exist.
 * @returns {Promise<string|null>} The new session ID, or null if tables unavailable
 */
async function createSession() {
  ...
  if (error) {
    logger.error('Failed to create chat session', { error: error.message, code: error.code, details: error.details });
    return null;
  }
  return data.id;
}
...
/**
 * Save a message to a session. Silently skips if sessionId is null.
 ...
 * Falls back to stateless mode if chat tables are unavailable.
 *
...
async function chatWithEpisodes(message, options = {}) {
  ...
  // Try to create/use a session — gracefully degrade if tables don't exist
  if (!sessionId) {
    sessionId = await createSession();
    if (sessionId) {
      logger.info('New chat session created', { sessionId });
    } else {
      logger.warn('Chat session tables unavailable — running stateless');
    }
  }

  // Load prior history from DB (returns [] if session is null or tables missing)
  ...
  // Save user message (skips if no session)
  ...
  logger.info('Calling generateChat', {
    messageCount: messages.length,
    contextLength: episodeContext.length,
  });
  ...
  // Save assistant reply (skips if no session)
